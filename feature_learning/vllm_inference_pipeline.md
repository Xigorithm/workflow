...
大语言模型（LLM）的推理本质是**自回归生成**（Autoregressive Generation）：给定 prompt tokens，模型逐步预测下一个 token。每一步的预测概率通过 softmax 归一化得到：
...
| 阶段 | 输入 | 计算特征 | 瓶颈 |
|------|------|----------|------|
| **Prefill** | 整个 prompt（n 个 tokens） | 大规模矩阵乘法，高并行度 | Compute-bound |
...
sequenceDiagram
    ...
    participant FE as AsyncLLM 前端
    participant EC as EngineCore 后端
    ...
    FE->>EC: EngineCoreRequest via ZMQ
    ...
    EC-->>FE: EngineCoreOutputs via ZMQ
...
| 8 | Logits 计算 | LM Head 将 hidden states 映射到词表空间 | logits |
...
用户通过 HTTP 请求访问 API Server，FastAPI 框架将请求路由到对应的处理函数。API Server 将 OpenAI 格式的 Chat 消息转换为引擎可处理的格式，然后调用 `AsyncLLM.generate()` 启动生成流程。
...
离线推理通过 `LLM` 类直接调用，不经过 HTTP 层。内部使用同步的 `LLMEngine`，循环调用 `step()` 直到所有请求完成。
...
class LLM:
    def generate(self, prompts, sampling_params, ...):
        ...
        # 2. _run_engine() 内部循环调用 llm_engine.step()
        #    直到所有请求完成
```

### 2.1.3 AsyncLLM.generate() — 异步生成入口
...
输入预处理的核心任务是将用户提供的各种形式的输入（文本、token IDs、embeds）统一转换为引擎内部的标准格式 `EngineCoreRequest`。
...
flowchart LR
    ...
    subgraph Preprocess["InputProcessor"]
        D["校验 SamplingParams"]
        E["Tokenization<br/>（如需要）"]
        F["多模态特征提取"]
        G["输入长度校验"]
    end
    
    subgraph Output["引擎内部格式"]
        H["EngineCoreRequest"]
    end
...
vLLM v1 采用**前端/后端进程分离**的设计。前端进程处理 HTTP 连接、Tokenization 和 Detokenization（CPU 密集型），后端进程运行调度器和 GPU 执行（GPU 密集型）。两者通过 **ZMQ IPC** 通信，避免 Python GIL 对 GPU 执行的阻塞。
...
> **关键洞察**：前后端分离使得 Tokenization/Detokenization 的 CPU 开销不会阻塞 GPU 执行，同时 ZMQ 的 msgpack 序列化保证了高效的跨进程数据传输。
...
### 3.1.1 step() 方法 — 推理引擎的心脏

EngineCore 的 `step()` 方法是整个推理引擎的核心。每次 step 完成一轮 **"调度 -> 执行 -> 采样 -> 更新"** 的完整循环：
...
class EngineCore:
    ...
    def step(self):
        ...
        # 第一步：调度 — 选择请求，分配 KV Cache
        scheduler_output = self.scheduler.schedule()
        
        # 第二步：执行 — 模型前向传播（异步提交到 GPU）
        future = self.model_executor.execute_model(
            scheduler_output, non_block=True
        )
        
        # 第三步：获取结构化输出语法掩码（与 GPU 执行并行）
        grammar_output = self.scheduler.get_grammar_bitmask(scheduler_output)
        
        # 第四步：等待 GPU 执行完成，获取结果
        model_output = future.result()
        if model_output is None:
            # 第五步：采样 — 从 logits 中采样 token
            model_output = self.model_executor.sample_tokens(grammar_output)
        
        # 第六步：更新 — 处理模型输出，生成 EngineCoreOutputs
...
> **关键洞察**：`execute_model()` 使用 `non_block=True` 异步提交到 GPU，同时 CPU 端并行计算 `grammar_bitmask`，实现了 CPU/GPU 的 overlap。

## 3.2 Scheduler 核心算法

### 3.2.1 统一 Token-Budget 调度模型

vLLM v1 的调度器采用**统一 token-budget 模型**，不区分 "Prefill 阶段" 和 "Decode 阶段"。每个请求只需让 `num_computed_tokens` 追上 `num_tokens_with_spec` 即可：
...
flowchart TD
    ...
    CHECK_RUNNING -->|"有请求 且 budget > 0"| CALC_NEW["计算 num_new_tokens"]
    ...
    CHECK_WAITING -->|"有请求 且 budget > 0"| PREFIX["查询 Prefix Cache"]
    PREFIX --> CALC_PREFILL["计算 prefill token 数<br/>cap by token_budget"]
    CALC_PREFILL --> ALLOC_PREFILL["分配 KV Cache Slots"]
    ALLOC_PREFILL -->|"成功"| ADD_RUNNING["加入 RUNNING 列表"]
    ALLOC_PREFILL -->|"失败"| BREAK_LOOP["停止调度"]
...
### 3.2.3 Phase 1：调度 RUNNING 请求

对于已在运行的请求（包括 decode 和进行中的 chunked prefill），计算需要调度的 token 数：
...
while req_index < len(self.running) and token_budget > 0:
    ...
    # 计算需要调度的 token 数
    # 对于 decode 请求，通常为 1（加上投机解码的 tokens）
    # 对于 chunked prefill，为剩余的 prompt tokens
    num_new_tokens = (
...
### 3.2.4 Phase 2：调度 WAITING 请求

对于等待中的新请求（prefill），先查询 Prefix Cache 命中，再分配 KV Cache：
...
while self.waiting and token_budget > 0:
    ...
    # Chunked Prefill 控制：
    # 如果禁用 chunked prefill 且 prompt 超过 budget，直接停止调度
...
### 3.2.5 调度后更新 — Chunked Prefill 的关键
...
def _update_after_schedule(self, scheduler_output):
    for req_id, num_tokens in scheduler_output.num_scheduled_tokens.items():
        ...
        # 推进 num_computed_tokens
        # 这使得 Chunked Prefill 在下一个 step 能从断点继续
...
### 3.3.1 PagedAttention Block 管理

vLLM 将 KV Cache 划分为固定大小的 Block（默认 16 tokens），通过 `BlockPool` 管理物理 Block 的分配和回收：

```mermaid
flowchart LR
    subgraph BlockPool["BlockPool（物理 Block 管理）"]
        FBQ["FreeKVCacheBlockQueue<br/>双向链表 LRU"]
        CACHE["BlockHashToBlockMap<br/>Prefix Cache 索引"]
        BLOCKS["KVCacheBlock 数组<br/>物理 Block"]
    end
    ...
    FBQ -->|"popleft 分配"| BLOCKS
    BLOCKS -->|"free_blocks 释放"| FBQ
    CACHE -->|"get_cached_block 查找"| BLOCKS
    BLOCKS -->|"cache_full_blocks 缓存"| CACHE
```

### 3.3.2 KVCacheManager 核心方法
...
class KVCacheManager:
    ...
    def allocate_slots(self, request, num_new_tokens, ...):
        ...
        if num_needed > available:
            return None  # 分配失败，触发抢占
        # 3. 分配新 blocks
        self.coordinator.allocate_new_computed_blocks(...)
        self.coordinator.allocate_new_blocks(...)
        # 4. 缓存满 blocks（提交 hash 到 Prefix Cache）
        self.coordinator.cache_blocks(request, ...)
```

### 3.3.3 Prefix Caching 原理

每个完整的 Block 通过链式 hash 标识，使得相同前缀的 Block 可以被复用：
...
| 4 | 返回 cache_hit_length = 2 x block_size | 只需计算剩余 tokens |

### 3.3.4 Block 驱逐策略

`FreeKVCacheBlockQueue` 使用双向链表实现 LRU 驱逐。释放的 Block 加入队列尾部（最后被驱逐），分配时从队列头部取出（最久未使用）：
...
Chunked Prefill 将长 prompt 的 Prefill 拆分为多个 step，与 Decode 请求混合执行，避免长 prompt 阻塞其他请求的 Decode：

```mermaid
flowchart LR
    subgraph Step1["Step 1: Prefill chunk 1"]
        P1["Request A<br/>tokens 0-511<br/>Prefill"]
        D1["Request B<br/>1 token<br/>Decode"]
    end
    
    subgraph Step2["Step 2: Prefill chunk 2"]
...
### 4.1.1 Worker 与 ModelRunner 的关系

Worker 是 GPU 工作线程的入口，负责管理 ModelRunner 和处理 Pipeline Parallelism 通信：
...
## 4.2 GPUModelRunner.execute_model() — 三阶段执行

ModelRunner 的 `execute_model()` 是整个 GPU 执行的核心，分为三个阶段：

```mermaid
flowchart TD
    subgraph Preprocess["Phase 1: Preprocess（输入准备）"]
        ...
        PP["_preprocess()<br/>多模态编码 和 Embedding"]
    end

    subgraph Forward["Phase 2: Model Forward（模型前向传播）"]
        FC["set_forward_context()<br/>设置全局 ForwardContext"]
        MF["_model_forward()<br/>self.model(input_ids, positions, ...)"]
    end

    subgraph Postprocess["Phase 3: Postprocess（后处理）"]
...
### 4.2.1 Phase 1: 输入准备 — `_prepare_inputs()`

这是将调度决策转化为 GPU 可消费的 Tensor 的关键步骤：

```python
# 文件: vllm/v1/worker/gpu_model_runner.py
def _prepare_inputs(self, scheduler_output, num_scheduled_tokens):
    # 1. 提交 block table 到 GPU（异步拷贝，与后续 CPU 操作 overlap）
    self.input_batch.block_table.commit_block_table(num_reqs)
    
    # 2. 构建请求索引
    # 例: 3 个请求分别有 [2, 5, 3] 个 tokens
    ...
    # 例: 请求 0 已计算 100 tokens，本次调度 2 个
    # -> positions = [100, 101]
    positions_np = (
...
> **关键洞察**：`_prepare_inputs()` 使用预分配的 CPU pinned memory 和 GPU buffer，通过 `torch.index_select` 高效 gather input_ids，避免了每次 step 重新分配内存的开销。

### 4.2.2 Phase 2: 模型前向传播 — Forward Context

vLLM 使用全局 `ForwardContext` 将 Attention 元数据传递给模型的每个 Attention 层，避免了在模型 forward 签名中传递大量参数：

```python
# 文件: vllm/v1/worker/gpu_model_runner.py
# 设置全局 ForwardContext 并执行模型前向
with set_forward_context(
    attn_metadata,           # 每层的 Attention 元数据
    self.vllm_config,
    num_tokens=num_tokens_padded,
    cudagraph_runtime_mode=cudagraph_mode,
    slot_mapping=slot_mappings,  # 每层的 KV Cache slot 映射
):
...
class FlashAttentionImpl:
    def forward(self, query, key, value, kv_cache):
        ...
        # 写入 KV Cache（新 token 的 K/V 存入对应 slot）
        kv_cache[slot_mapping] = torch.cat([key, value], dim=-1)
        
        # 执行 FlashAttention（读取历史 KV Cache）
...
# 提取需要计算 logits 的 hidden states
# 每个请求只取最后一个 token 的 hidden state（用于预测下一个 token）
sample_hidden_states = hidden_states[logits_indices]

# LM Head: hidden_states -> logits (vocab_size 维)
...
> **性能提示**：CUDA Graph 可以将 Decode 阶段的 kernel launch 开销从数百微秒降低到几微秒，对 TPOT 有显著影响。

## 4.4 Pipeline Parallelism 支持
...
采样是将模型输出的 logits 分布转换为具体 token ID 的过程。vLLM 的 Sampler 实现了完整的采样流水线：

```mermaid
flowchart TD
    LOGITS["logits<br/>batch x vocab_size"] --> RAW["保存原始 logprobs"]
    RAW --> F32["转 float32"]
    F32 --> LP["apply_logits_processors()"]
    
    subgraph LogitsProcessors["Logits Processors 链"]
...
class Sampler(nn.Module):
    def forward(self, logits, sampling_metadata):
        ...
        # 3. 应用 logits processors 链
        #    按顺序：白名单 -> 黑名单 -> 非 argmax 不变处理器 -> 惩罚
        ...
        # 5. 收集 logprobs（top-k + 采样 token 的 logprob + rank）
...
vLLM 使用 Gumbel-max trick 替代 `torch.multinomial`，避免 CPU-GPU 同步：
...
def random_sample(self, probs, generators):
    ...
    # 优势：完全在 GPU 上执行，无需 CPU-GPU 同步
    exponential_noise = torch.empty_like(probs).exponential_(
        1.0, generator=gen
    )
...
> **关键洞察**：`torch.multinomial` 会触发 CPU-GPU 同步，而 Gumbel-max trick 通过除以指数分布噪声再取 argmax 的方式，完全在 GPU 上完成采样，对 Decode 阶段的延迟至关重要。
...
| `repetition_penalty` | float | 1.0 | 重复惩罚（大于 1 抑制重复） |
| `frequency_penalty` | float | 0.0 | 频率惩罚（按出现次数） |
| `presence_penalty` | float | 0.0 | 存在惩罚（是否出现过） |
| `max_tokens` | int | 16 | 最大生成 token 数 |
| `stop` | list of str | None | 停止字符串 |
| `stop_token_ids` | list of int | None | 停止 token ID |
...
### 6.1.1 update_from_output() — 处理模型输出

模型执行完成后，Scheduler 需要将采样结果追加到请求中，并检查停止条件：
...
def update_from_output(self, scheduler_output, model_runner_output):
    ...
    for i, req_id in enumerate(model_runner_output.req_ids):
        ...
        # 检查停止条件
        finish_reason = check_stop(request, self.max_model_len, token_ids)
...
## 6.2 OutputProcessor — Detokenization
    ...
    def process_outputs(self, engine_core_outputs):
        ...
        for engine_core_output in engine_core_outputs:
            ...
            request_output = req_state.make_request_output(
                engine_core_output, stop_string
            )
            
            # 4. 推送到请求队列（供 generate() 消费）
            req_state.output_queue.put(request_output)
            
            # 5. 如果完成，清理状态
            if engine_core_output.finish_reason is not None:
                self._free_request(req_state)
```

### 6.2.2 增量 Detokenizer

增量 Detokenizer 的核心思想是：不需要每次将整个 token 序列重新解码，而是只解码新增的 token，并处理 BPE 合并等边界情况：
...
sequenceDiagram
    ...
    AIO->>AIO: render_chat_request() 转为 token_ids
    AIO->>ALM: generate(engine_input, sampling_params)
    ALM->>IP: process_inputs() 得到 EngineCoreRequest
    IP-->>ALM: EngineCoreRequest
    ALM->>OP: add_request() 创建 RequestOutputCollector
    ALM->>ZMQ: add_request_async() 序列化发送
    ZMQ->>EC: 反序列化为 Request 对象
    EC->>SCH: add_request() 加入 waiting 队列

    loop 每个 EngineCore Step
        EC->>SCH: schedule()
        SCH->>KVM: get_computed_blocks() 查 prefix cache
    ...
    OP-->>ALM: RequestOutput 推入队列
    ALM-->>AIO: yield RequestOutput 流式返回
...
flowchart TD
    ...
    C --> D["LLMEngine.add_request()<br/>InputProcessor 转为 EngineCoreRequest"]
    D --> E["_run_engine() 主循环"]
...
classDiagram
    class EngineCoreRequest {
        +request_id: str
        +prompt_token_ids: list of int
        +sampling_params: SamplingParams
        +mm_features: list of MultiModalFeatureSpec
        +arrival_time: float
    }

    class SchedulerOutput {
        +scheduled_new_reqs: list of NewRequestData
        +scheduled_cached_reqs: CachedRequestData
        +num_scheduled_tokens: dict of str to int
        +total_num_scheduled_tokens: int
        +scheduled_spec_decode_tokens: dict
        +finished_req_ids: set of str
    }

    class ModelRunnerOutput {
        +req_ids: list of str
        +sampled_token_ids: list of list of int
        +logprobs: list of LogprobsLists
        +prompt_logprobs_dict: dict
    }

    class EngineCoreOutput {
        +request_id: str
        +new_token_ids: list of int
...
| 数据结构 | 生产者 | 消费者 | 说明 |
|----------|--------|--------|------|
| `EngineCoreRequest` | InputProcessor | EngineCore | 前端发送到后端的序列化请求 |
| `SchedulerOutput` | Scheduler | ModelRunner | 调度决策：哪些请求执行多少 tokens |
| `ModelRunnerOutput` | ModelRunner | Scheduler | 模型执行结果：采样 token + logprobs |
| `EngineCoreOutput` | Scheduler | OutputProcessor | 每个请求的输出：新 token + 完成原因 |
| `RequestOutput` | OutputProcessor | API Server / 用户 | 最终输出：文本 + logprobs + 完成状态 |
...
## 9.2 Persistent Batch 优化

`InputBatch` 预分配所有 CPU/GPU tensors，请求的添加和移除只修改对应 slot，避免每次 step 重新分配内存：
...
class InputBatch:
    ...
    def add_request(self, request):
        ...
        self.token_ids_cpu_tensor[idx, :len(request.token_ids)] = (
            request.token_ids
        )
