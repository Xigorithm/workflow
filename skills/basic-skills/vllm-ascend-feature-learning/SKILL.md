---
name: vllm-ascend-feature-learning
description: "Generate comprehensive Chinese technical learning documents for vllm-ascend (vLLM Ascend NPU plugin) features and modules. Produces deep-dive code walkthrough documents with Mermaid architecture/flow diagrams, GPU-vs-NPU comparison tables, plugin integration analysis, code snippets, and configuration guides. Output is saved as Markdown to the skill's outputs/ directory. TRIGGER when: user asks to learn about a vllm-ascend feature, module, or NPU-specific capability (e.g., \"我想了解 vllm-ascend 的 xxx\", \"帮我生成 vllm-ascend xxx 的学习文档\", \"generate a learning guide for vllm-ascend's xxx feature\", \"vllm-ascend xxx 特性分析\", \"学习 vllm-ascend 的 patch 系统\", \"了解 Ascend NPU 的 ACL Graph\"). Also trigger for: plugin architecture, patch system, NPUPlatform, attention backends (MLA/SFA/DSA), FlashComm, MC2, CaMem allocator, EPLB, ACL Graph, spec decode on NPU, quantization on Ascend, or any vllm-ascend specific topic. DO NOT TRIGGER when: user asks about upstream vLLM features (use vllm-feature-tutorial instead), non-vLLM projects, or simple Q&A without requesting a learning document."
---
